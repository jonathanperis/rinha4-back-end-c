...
int main(int argc, char **argv) {
    ...
    while ((line = read_line(f, &cap)) != NULL) {
        ...
        int expected_approved = expected < 3;
        int actual_approved = actual < 3;
        if (expected_approved && !actual_approved) ++fp;
        if (!expected_approved && actual_approved) ++fn;
        if (actual != expected) {
            if (mismatches < 40) {
                fprintf(stderr, "mismatch line=%llu expected=%d actual=%u expected_approved=%d actual_approved=%d\n",
                        (unsigned long long)(total + 1), expected, actual, expected_approved, actual_approved);
            }
            ++mismatches;
        }
        ++total;
